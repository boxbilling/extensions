...
class Box_Mod_Servicecentovacast_Service
{
    ...
    public function toApiArray($row)
    {
        ...
        $row['pass'] = Box_Tools::decrypt($row['pass'], $this->_salt);
        return $row;
    }
    ...
    {
        $model = R::dispense('service_centovacast');
    ...
    public function activate($order, $model)
    {
        ...
        $password       = ( isset($oc['password']) && !empty($oc['password']) ) ? $oc['password'] : Box_Tools::generatePassword();
        $sourcepassword = Box_Tools::generatePassword();
    ...
    private function _genUsername($client)
    {
        ...
        $exists = R::getCell($sql, array('u'=>$u));
        if($exists) {
            $u = $u. rand(1, 100); 
        }
        return $u;
    ...
    private function _apiServerCall($model, $method, $arguments = array(), $admin = false)
    {
        ...
        $account_password = Box_Tools::decrypt($model->pass, $this->_salt);
        $server             = $this->getServer($model->server_id);
    ...
    {
        return Box_Tools::encrypt($password, $this->_salt);
    }
}